...
| 组件 | 职责 | 触发方式 |
|------|------|---------|
| `scripts/ingest.sh` | 检测 raw 新文件 → Claude CLI 处理 → 写入 wiki → Git 闭环 | 手动触发 |
| `wikiapp/` | Web 展示 + 摄入 UI + Lint API | Docker 容器 + 宿主机 API |
| `99-wiki/.raw-coverage` | raw 覆盖率追踪（状态 + 关联页面 + 日期） | ingest.sh / AI 维护 |
...
- 有孤立页面（没有入链）吗？
- 有知识空白需要补充吗？
- **知识空白扫描** — 对比 `99-wiki/.raw-coverage`，识别有 raw 素材但未提炼的领域
